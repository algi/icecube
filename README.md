...
## Error handling
Error handling in Cocoa is very robust and nicely implemented. If you would like to see how to create nice error sheet for user, head to `MBMavenServiceTask` clas, where you can see how to create proper `NSError` object, which you can pass to the application. For error handling itself, see method `-mavenTaskDidFinishSuccessfully:error` in class `MBTaskRunnerWindowController`. As you can see, there’s some weird bug there, because `NSModalResponse` should be enum of possible values (which are negative integer values), but the method itself gives you positive integer… More you provide information in NSError userDictionary, the more Cocoa can do for you in error dialog.
